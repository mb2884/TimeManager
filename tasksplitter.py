...
def split_tasks(user_id, title, start, end, length, task_id,  calendar_events):
    ...
    for event in events:
        ...
        while current_day < event_end.date():
            event_times[current_day] += (datetime.datetime.combine(current_day, datetime.time(hour=work_end_time)) - event_start).total_seconds() / 60
            event_start = datetime.datetime.combine(current_day, datetime.time(hour=work_start_time))
            current_day += datetime.timedelta(days=1)
        event_times[current_day] += (event_end - event_start).total_seconds() / 60
    ...
    while time_left_in_task > 0:
        ...
        min_day = [y[0] for y in sorted(event_times.items(), key = lambda x: x[1])[:k]][-1]
        # Check if the day is the first day in which case we need to start at the start time of the task
        if min_day == start.date():
            current_day = start.replace(tzinfo=nytz)
        else:
            current_day = datetime.datetime.combine(min_day, datetime.time(hour=work_start_time)).replace(tzinfo=nytz)
        print("Current day: ", current_day)
        ...
        while current_day < datetime.datetime.combine(min_day, datetime.time(hour=work_end_time)).replace(tzinfo=nytz) and time_left_in_task > 0:
            ...
            if not overlap:
                ...
                chunk_end = current_day + datetime.timedelta(minutes=min(ideal_chunk_length, time_left_in_task))
                database.addEvent(user_id, title, chunk_start.replace(tzinfo=None), chunk_end.replace(tzinfo=None), False, task_id, "#4CAF50", None, None, None)
                # add to events
                events.append({'title': title, 'start': chunk_start.strftime("%Y-%m-%dT%H:%M:%S.000Z"), 'end': chunk_end.strftime("%Y-%m-%dT%H:%M:%S.000Z"), 'allDay': False, 'id': None, 'parentTaskID': task_id, 'color': "#4CAF50", 'daysOfWeek': None, 'startRecur': None, 'endRecur': None, 'startTime': chunk_start.time().isoformat(), 'endTime': chunk_end.time().isoformat()})
                time_left_in_task -= (chunk_end - chunk_start).total_seconds() / 60
                # Update the event_times dictionary
                event_times[min_day] += (chunk_end - chunk_start).total_seconds() / 60
                break
            current_day += datetime.timedelta(minutes=ideal_chunk_length + event_padding)
        k += 1   
